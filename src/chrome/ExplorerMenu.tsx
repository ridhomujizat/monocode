...
type Props = {
  ...
  /** Hover on an item; used by callers that cascade submenus themselves. */
    ...
    return (
      <button
        ...
        onPointerDown={(e) => {
          ...
          if (pickOnMouseDown && !item.disabled && !item.submenu?.length) {
            e.preventDefault();
            onPick(item.id);
            onClose();
          }
